    ...
    while True:
        ...
        for op in operations:
            ...
            if best_input_amount is None:
                best_input_amount = graph.tradeSize
            if best_input_amount < graph.tradeSize * 0.01:
                best_input_amount = graph.tradeSize * 0.1
            min_output = simulate_execute_arbitrage(graph, op, best_input_amount)
            if should_execute(op, min_output):
                print("executing a trade")
                execute_arbitrage(op, min_output, graph.tradeSize)
